...
public class Message_Fragment extends Fragment{
    ...
    private TextView data_name,data_eat0,data_eat1,data_eat2,data_eat3,data_count;
    private TextView t_qualify,t_max,t_min,pie_avg;
    private TextView name_0,name_1,name_2,name_3;
    private TextView dcolor0,dcolor1,dcolor2,dcolor3;
    String uname;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        pieChart =view.findViewById(R.id.pieChar);
        combinedChart =view.findViewById(R.id.combin_chart);
        data_eat0=view.findViewById(R.id.kongfu);
        data_eat1=view.findViewById(R.id.eat1h);
        data_eat2=view.findViewById(R.id.eat2h);
        data_eat3=view.findViewById(R.id.eat3h);
        name_0 =view.findViewById(R.id.name_0);
        name_1 =view.findViewById(R.id.name_1);
        name_2 =view.findViewById(R.id.name_2);
        name_3 =view.findViewById(R.id.name_3);
        dcolor0 =view.findViewById(R.id.dcolor0);
        dcolor1 =view.findViewById(R.id.dcolor1);
        dcolor2 =view.findViewById(R.id.dcolor2);
        dcolor3 =view.findViewById(R.id.dcolor3);
        data_count=view.findViewById(R.id.data_count2);
        t_qualify=view.findViewById(R.id.t_qualify);
        t_max=view.findViewById(R.id.t_max);
        t_min=view.findViewById(R.id.t_min);
        pie_avg=view.findViewById(R.id.pie_avg);
        //数据传递
        data_name = view.findViewById(R.id.data1);
        Bundle bundle =getActivity().getIntent().getExtras();
        uname =bundle.getString("uname");
        data_name.setText(uname);
        //获取用户信息
        gapResultDao =new GapResultDao();
        mainhandler = new Handler(getActivity().getMainLooper());//获取主线程
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                mainhandler.post(new Runnable() {
                    @Override
                    public void run() {
//                        Toast.makeText(Message_Fragment.this, "信息查询成功！", Toast.LENGTH_SHORT).show();
                        if (gapResultList!=null){
                            //num  共测量次数，用于计算达标率
                            int num=gapResultList.size();
                            if (num!=0) {
                                LongLog.printMsg(gapResultList.size() + "--该用户共测量的记录次数--");
                                data_count.setText("检测次数： " + num + "次");
                                ...
                                String eat0 = (String) name_0.getText();
                                String eat1 = (String) name_1.getText();
                                String eat2 = (String) name_2.getText();
                                String eat3 = (String) name_3.getText();
                                LongLog.printMsg("获取名字" + eat0 + eat1 + eat2 + eat3);
                                LongLog.printMsg("zhi:" + list_line.toString());
                                if (eat0.equals("空腹")) {
                                    LongLog.printMsg("空腹:" + eat0);
                                    GradientDrawable background = (GradientDrawable) dcolor0.getBackground();
                                    if (int_kong < 4.4) {
                                        background.setColor(Color.parseColor("#ffef5d"));
                                    } else if (int_kong > 8.2) {
                                        background.setColor(Color.parseColor("#ff6a57"));
                                    } else {
                                        background.setColor(Color.parseColor("#2ee79f"));
                                    }
                                }
                                if (eat1.equals("餐后1h")) {
                                    GradientDrawable background1 = (GradientDrawable) dcolor1.getBackground();
                                    if (int_1h < 6.7) {
                                        background1.setColor(Color.parseColor("#ffef5d"));
                                    } else if (int_1h > 12.7) {
                                        background1.setColor(Color.parseColor("#ff6a57"));
                                    } else {
                                        background1.setColor(Color.parseColor("#2ee79f"));
                                    }
                                }

                                if (eat2.equals("餐后2h")) {
                                    GradientDrawable background2 = (GradientDrawable) dcolor2.getBackground();
                                    if (int_2h < 5.0) {
                                        background2.setColor(Color.parseColor("#ffef5d"));
                                    } else if (int_2h > 11.0) {
                                        background2.setColor(Color.parseColor("#ff6a57"));
                                    } else {
                                        background2.setColor(Color.parseColor("#2ee79f"));
                                    }
                                }
                                if (eat3.equals("餐后3h")) {
                                    GradientDrawable background3 = (GradientDrawable) dcolor3.getBackground();
                                    if (int_3h < 4.4) {
                                        background3.setColor(Color.parseColor("#ffef5d"));
                                    } else if (int_3h > 9.9) {
                                        background3.setColor(Color.parseColor("#ff6a57"));
                                    } else {
                                        background3.setColor(Color.parseColor("#2ee79f"));
                                    }
                                }
                            }
                        }
...
//        lineChart.getAxisRight().setEnabled(false);
        ...
        combinedChart.setScaleEnabled(false);//不可缩放
        combinedChart.getDescription().setEnabled(false);
    ...
    private void setData() {    //饼状图
        ...
        pieChart.setDrawHoleEnabled(true);/**中间是否是空的*/
        //数据更新
        pieChart.notifyDataSetChanged();
        pieChart.invalidate();
        //动画（如果使用了动画可以则省去更新数据的那一步）
        pieChart.animate(); //折线在Y轴的动画  参数是动画执行时间 毫秒为单位
    }
}
